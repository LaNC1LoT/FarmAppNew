...
insert into api.ApiMethodRoles (ApiMethodId, RoleId)
select a.Id, t.roleId from api.ApiMethods a
cross join @t t


update api.ApiMethods set IsNeedAuthentication = 0 where id in (41, 42)
update api.ApiMethods set PathUrl = REPLACE(PathUrl, '?', '')


select * from api.ApiMethodRoles
select * from api.ApiMethods


--INSERT INTO api.ApiMethods (ApiMethodName, Description, PathUrl, HttpMethod)
--values ('GetChartSales', 'График продаж', '/api/api/Charts/Sales', 'GET')
...
--SELECT a.Id, t.RoleId FROM api.ApiMethods a
--CROSS JOIN @t t
--WHERE ApiMethodName = 'GetChartSales'


INSERT INTO api.ApiMethods (ApiMethodName, Description, PathUrl, HttpMethod, IsNeedAuthentication) VALUES
(N'GetStocks', N'Получение списка', N'/api​/Stocks', 'GET', 1),
(N'PutStocks', N'Редактирование', N'/api​/Stocks', 'PUT', 1),
(N'PostStocks', N'Добавление', N'/api​/Stocks', 'POST', 1),
(N'DeleteStocks', N'Удаление', N'/api​/Stocks', 'DELETE', 1)
...
--SELECT a.Id, t.roleId FROM api.ApiMethods a
--cross join @t t 
--	where PathUrl = '/api/Stocks'

INSERT INTO api.ApiMethods (ApiMethodName, Description, PathUrl, HttpMethod)
values ('GetChartStocks', 'График склада', '/api/Charts/Stocks', 'GET')
...
SELECT a.Id, t.RoleId FROM api.ApiMethods a
CROSS JOIN @t t
WHERE ApiMethodName = 'GetChartStocks'
